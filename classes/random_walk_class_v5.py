...
class RandomWalkV5(BaseAlgorithm):
    def __init__(self):
        self.y_score = []
        self.y_true = []
    ...
    def predict(
        ...
        for positive_protein, positive_go in zip(
            ...
            D.remove_edge(positive_protein, positive_go)
            p = nx.pagerank(D, alpha=0.7, personalization={positive_go: 1})
            ...
            D.add_edge(positive_protein, positive_go, type="protein_go_term")
        ...
        for negative_protein, negative_go in zip(
            ...
            p = nx.pagerank(D, alpha=0.7, personalization={negative_go: 1})
        ...
        df.to_csv(
            Path(output_path, "random_walk_data_v5.csv"),
            index=False,
            sep="\t",
        )
